...
unsure about the namespace, that's for sure, but ideally, the commands should
all be modules, rather than xml files (or whatever).  Commands are located as
plugins using L<Module::Pluggable>
...
I also found something called L<Term::UI> and L<Term::UI>.
